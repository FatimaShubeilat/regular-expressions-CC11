...
const validatePin = (pin) => {
  // Solution code here...
  let regularExpressions = /^[0-9]{4}$/;
  return regularExpressions.test(pin);
};
...
const validateEmail = (email) => {
  // Solution code here...
  let regularExpressions = /^\w+([\.-]?\w+)*@\w+.((org)|(net)|(com))$/;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let regularExpressions = /^(\d{3}|\(\d{3}\))(| |-|)\d{3}(| |-)\d{4}$/;
  return regularExpressions.test(phoneNumber);
};
